...
import { SlideDocument, SlideEntityM } from './slide.entity';
import { UserDocument, UserEntityM } from './user.entity';
...
class RequestedParametersRespondentDataProperty {
  ...
  @Prop({ required: true, type: mongoose.Schema.Types.ObjectId, ref: Variable.name })
  variable: string;
...
export class ExperimentEntityM {
  @Prop({
    required: true,
  })
  title: string;
...
export type ExperimentDocument = HydratedDocument<ExperimentEntityM>;
export const ExperimentSchema = SchemaFactory.createForClass(ExperimentEntityM);

ExperimentSchema.virtual('slides', {
  ref: SlideEntityM.name,
  localField: '_id',
